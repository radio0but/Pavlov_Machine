...
String htmlCode =
    ...
    "<div class='column'>"
    // Set Feeding Time
    "<h2 onclick='toggleContent(this)'>Set Feeding Time</h2>"
    "<div>"
    ...
    "</form>"
    "</div>" // Close the collapsible content div
    "</div>"
    // Toggle feeding
    "<div class='column'>"
    "<h2 onclick='toggleContent(this)'>Feed</h2>"
    "<div>"
    "<form action='/feed' method='post'>"
    "<input type='submit' value='Feed'>"
    "</form>"
    "</div>"
    "<h2 onclick='toggleContent(this)'>Gate controls</h2>"
    "<div>"
    // Servo Controls Open and Close Gate
    ...
    "</form>"
    "</div>"
    "<h2 onclick='toggleContent(this)'>Carousel Control</h2>"
    "<div>"
    ...
    "</div>"
    "<div class='column'>"
    "<h2 onclick='toggleContent(this)'>Configurations</h2>"
    "<div>"
    ...
    "</form>"
    "<h2 onclick='toggleContent(this)'>Motors Settings</h2>"
    "<div>"
    "<h3>Stepper(Caroussel)</h3>"
    "<form action='/setstepper' method='post'>"
    "Steps per Rotation: <input type='number' name='stepsPerRotation' "
    "value='2048' min='1'>"
    "<input type='submit' value='Set'>"
    "</form>"
    "<h3>Servo(Gate)</h3>"
    "<form action='/setangles' method='post'>"
    "Open Angle: <input type='number' name='openAngle'value='90'>"
    "Closed Angle: <input type='number' name='closedAngle'value='0'>"
    "<input type='submit' value='Set'>"
    "</form>"

    "</div>"
    "<h2 onclick='toggleContent(this)'>Theme Configuration</h2>"
    "<div>"
    ...
    "<h3 onclick='toggleContent(this)'>Image Setting</h3>"
    "<div>"
    ...
    "</div>"
    "</div>"
    "<h2 onclick='toggleContent(this)'>System</h2>"
    "<div>"
...
void modifyHtml(Preferences &preferences) {
  ...
  String modifiedCSS =
      "body { font-family: Tahoma, sans-serif; background-color: " + ibgColor +
      "; color: " + itextColor +
      "; display: flex; flex-direction: column; align-items: center; } "
      ".header { padding: 10px; text-align: center; background-color: " +
      iheaderBgColor +
      "; color: white; width: 80%;font-size: 2vw; border: " + iborderSize +
      "px solid " + iborderColor +
      "; border-radius: 25px;  } "
      ".header a {color:" +
      itextColor +
      ";}"
      ".content { display: flex; flex-wrap: wrap; justify-content: "
      "space-between; width: 100%; padding: 20px; font-size: 1vw; } "
      ".column { flex: 1; padding: 20px; width: 90%;border: " +
      iborderSize + "px solid " + iborderColor +
      "; border-radius: 25px;  }"
      ...
      ibuttonColor + "; width: 100%; color: " + itextColor +
      "; cursor: pointer; font-size: 1vw;border: " + iborderSize + "px solid " +
      iborderColor +
      "; border-radius: 25px; } "
      "input[type='submit']:hover { width: 100%; background-color: " +
      ibuttonColor + ";font-size: 1.5vw; border: " + iborderSize + "px solid " +
      iborderColor +
      "; border-radius: 25px; } "
      "input[type='time']{ background-color:" +
      ibgColor +
      "; width: 100%; color: white; cursor: pointer; font-size: 1vw;}"
      "@media screen and (max-width: 1024px) { .content { flex-direction: "
      "column; font-size: 3vw;} .column { width: 100%; } input[type='submit'] "
      "{   font-size: 3vw;  } "
      " input[type='submit']:hover {    font-size: 3.5vw;} input[type='time']{ "
 ...
 "body { font-family: Tahoma, sans-serif; background-color: black; color: "
  ...
  modifiedHtmlCode.replace("name='feedingTime1'>",
                           "name='feedingTime1' value='" + feedingTime1 + "'>");
  modifiedHtmlCode.replace("name='feedingTime2'>",
                           "name='feedingTime2' value='" + feedingTime2 + "'>");
  modifiedHtmlCode.replace("name='feedingTime3'>",
                           "name='feedingTime3' value='" + feedingTime3 + "'>");
  modifiedHtmlCode.replace("name='feedingTime4'>",
                           "name='feedingTime4' value='" + feedingTime4 + "'>");
  ...
  if (headerImageUrl != "") {
    modifiedHtmlCode.replace(
        "<h2 onclick='toggleContent(this)'>Set Feeding Time</h2>",
        "<img src='" + headerImageUrl +
            "' alt='Header Image' style='width:100%;'></div><div "
            "class='column'><h2 onclick='toggleContent(this)'>Set Feeding "
            "Time</h2>");
  } else {
    modifiedHtmlCode.replace(
        "<h2 onclick='toggleContent(this)'>Set Feeding Time</h2>",
